...
use crate::model::types::{DownloadPhase, DownloadProgress, ModelInfo, ModelStatus};
...
pub struct ModelManager {
    ...
    queue: tokio::sync::Mutex<DownloadQueue>,
}
...
impl ModelManager {
    ...
    pub fn new(model_dir: PathBuf) -> Arc<Self> {
        Arc::new(Self {
            ...
            queue: tokio::sync::Mutex::new(DownloadQueue {
    ...
    pub async fn list_models(&self) -> Vec<ModelInfo> {
        ...
        for def in builtin_models() {
            ...
            let (status, disk_bytes) = if let Some(progress) = downloads.get(&def.id) {
    ...
    /// Try to claim an active download slot. If the concurrency limit is
    /// reached, queues the request and returns `None`. If a slot is
    /// available, increments the active count and returns `Some(request)`
    /// for the caller to start the download.
    pub async fn try_claim_download_slot(
        &self,
    ...
    /// Called when a download task completes or fails. Decrements the active
    /// count and returns the next queued request (if any) with an active slot
    /// already claimed for it.
    pub async fn on_download_finished(&self) -> Option<QueuedDownloadRequest> {
        let mut q = self.queue.lock().await;
    ...
    /// Release an active download slot without popping from the queue.
    /// Used when a download fails to start after claiming a slot.
    pub async fn release_download_slot(&self) {
        let mut q = self.queue.lock().await;
        q.active = q.active.saturating_sub(1);
    }
    ...
    /// Check whether a model is currently being downloaded or queued.
    ...
    /// Cancel an in-progress or queued download. For active downloads, aborts
    /// the task and cleans up `.part` files. For queued downloads, simply
    /// removes from the queue. Returns `true` if an active slot was released
    /// (caller should check for next queued download).
    pub async fn cancel_download(&self, model_id: &str) -> Result<bool, AsrError> {
        // Check if it's queued (not yet started).
        {
            let mut q = self.queue.lock().await;
            if let Some(idx) = q.pending.iter().position(|r| r.model_id == model_id) {
                ...
                return Ok(false); // No active slot was released.
            }
        }

        // Otherwise, it's an active download — abort the task.
        ...
        self.release_download_slot().await;

        info!(model_id = %model_id, "active download cancelled");
        Ok(true) // Active slot released — caller should start next queued.
    }
