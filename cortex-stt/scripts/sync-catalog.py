...
own schema, resolves per-file sha256 from Hugging Face LFS metadata, and
regenerates MODELS.md. Run manually; review the diff before committing.
...
MODELS_MD_OUT = REPO_ROOT.parent / "MODELS.md"
...
def size_mb(size_bytes: int) -> str:
    return f"{size_bytes / (1024 * 1024):.0f} MB"


def render_models_md(catalog: dict) -> str:
    lines = [
        "# Supported Models",
        "",
        "> Generated by `cortex-stt/scripts/sync-catalog.py` — do not edit by hand.",
        f"> Source: {catalog['source']} (synced {catalog['generated_at']}).",
        "",
    ...
    for family in sorted(by_family):
        ...
        for m in sorted(by_family[family], key=lambda x: x["id"]):
            quants = ", ".join(
                (f"**{q['quant']}** ({size_mb(q['size_bytes'])})"
                 if q["quant"] == m["default_quant"]
                 else f"{q['quant']} ({size_mb(q['size_bytes'])})")
                for q in m["quants"]
            )
            streaming = "yes" if m["capabilities"]["streaming"] else "no"
            lines.append(
                f"| `{m['id']}` ({m['name']}) | {m.get('parameters') or '—'} "
                f"| {len(m['languages'])} | {streaming} | {quants} | {m.get('license') or '—'} |"
            )
        lines.append("")
    return "\n".join(lines)
...
def main() -> None:
    ...
    parser.add_argument("--models-md", default=str(MODELS_MD_OUT))
    ...
    Path(args.models_md).write_text(render_models_md(catalog) + "\n")

    print(f"\nWrote {args.out} ({len(catalog['models'])} models)", file=sys.stderr)
    print(f"Wrote {args.models_md}", file=sys.stderr)
    if skipped:
